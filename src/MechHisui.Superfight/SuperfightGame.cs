...
using MechHisui.Superfight.Collections;
...
namespace MechHisui.Superfight
{
    public sealed class SuperfightGame : GameBase<SuperfightPlayer>
    {
            ...
            : base(channel, players)
        {
            ...
            _debateTimer = new Timer(async cb => await StartVote(), null, Timeout.Infinite, Timeout.Infinite);
        ...
        private async Task EndVote(IList<PlayerVote> l)
        {
            ...
            await Channel.SendMessageAsync($"Voting has ended. The winner is **{winner.Player.User.Username}**.");
