            ...
            client.GetService<CommandService>().CreateCommand("ap")
                ...
                .Do(async cea =>
                {
                    ...
                    if (cea.Args.All(s => s == String.Empty))
                    {
                        ...
                        if (userap != null)
                        {
                            await cea.Channel.SendWithRetry($"{cea.User.Name} currently has {userap.CurrentAP} AP.");
                        }
                        else
                        {
                            await cea.Channel.SendWithRetry($"Currently not tracking {cea.User.Name}'s AP.");
                        }
                        return;
                    }
                    ...
                    if (Int32.TryParse(cea.Args[0], out startAmount) && TimeSpan.TryParseExact(cea.Args[1], @"m\:%s", CultureInfo.InvariantCulture, out startTime))
                    {
                        ...
                        await cea.Channel.SendWithRetry($"Now tracking AP for `{cea.User.Name}`.");
                    }
                    else
                    {
                        await cea.Channel.SendWithRetry("One or both arguments could not be parsed correctly.");
                    }
                });
        }
            ...
            client.GetService<CommandService>().CreateCommand("daily")
                ...
                .Do(async cea =>
                {
                    ...
                    else if (arg == "change")
                    {
                        ...
                        await cea.Channel.SendWithRetry($"Daily quests changing **ETA: {eta.Hours} {h} and {eta.Minutes} {m}.**");
                        return;
                    }
                    ...
                    {
                        await cea.Channel.SendWithRetry("Could not convert argument to a day of the week or Servant class. Please try again.");
                        return;
                    }
        ...
        public static void RegisterLoginBonusCommand(this DiscordClient client, IConfiguration config)
        {
            ...
            client.GetService<CommandService>().CreateCommand("login")
                ...
                .Do(async cea =>
                {
                    ...
                    string h = eta.Hours == 1 ? "hour" : "hours";
                    string m = eta.Minutes == 1 ? "minute" : "minutes";
                    await cea.Channel.SendWithRetry($"Next login bonus drop **ETA {eta.Hours} {h} and {eta.Minutes} {m}.**");
                });
            ...
            LoginBonusTimer = new Timer(async cb =>
            {
                ...
                await ((DiscordClient)cb).GetChannel(UInt64.Parse(config["FGO_general"]))
                    .SendWithRetry("Login bonuses have been distributed.");
            },
            client,
...
```"));
        ...
        public static void RegisterZoukenCommand(this DiscordClient client, IConfiguration config)
        {
            client.GetService<CommandService>().CreateCommand("zouken")
                ...
                .Do(async cea =>
                {
                    await cea.Channel.SendWithRetry("Friendly reminder that the Zouken CE doesn't trigger on suicides, so don't even think about pairing it with A-Trash.");
                });
        }
